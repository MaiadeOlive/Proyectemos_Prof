import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';

abstract class ThemeText {
  // H1 Poppins
  static TextStyle h1title45WhiteBold = TextStyle(
    fontSize: ScreenUtil().setSp(45),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.white,
    fontWeight: FontWeight.bold,
  );

  static TextStyle h1title45RedBold = TextStyle(
    fontSize: ScreenUtil().setSp(45),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.red,
    fontWeight: FontWeight.bold,
  );

  static TextStyle h1title45BlueBold = TextStyle(
    fontSize: ScreenUtil().setSp(45),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.blue,
    fontWeight: FontWeight.bold,
  );

  static TextStyle h1title45YellowBold = TextStyle(
    fontSize: ScreenUtil().setSp(45),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.yellow,
    fontWeight: FontWeight.bold,
  );

  static TextStyle h1title45WhiteNormal = TextStyle(
    fontSize: ScreenUtil().setSp(45),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.white,
    fontWeight: FontWeight.normal,
  );

  static TextStyle h1title45RedNormal = TextStyle(
    fontSize: ScreenUtil().setSp(45),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.red,
    fontWeight: FontWeight.normal,
  );

  static TextStyle h1title45BlueNormal = TextStyle(
    fontSize: ScreenUtil().setSp(45),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.blue,
    fontWeight: FontWeight.normal,
  );

  static TextStyle h1title45YellowNormal = TextStyle(
    fontSize: ScreenUtil().setSp(45),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.yellow,
    fontWeight: FontWeight.normal,
  );

  //H1 Ginto
  static TextStyle h1title45YellowBoldGinto = TextStyle(
    fontSize: ScreenUtil().setSp(45),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.yellow,
    fontWeight: FontWeight.bold,
  );

  //H2
  static TextStyle h2title35White = TextStyle(
    fontSize: ScreenUtil().setSp(35),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.white,
    fontWeight: FontWeight.bold,
  );

  static TextStyle h2title35Red = TextStyle(
    fontSize: ScreenUtil().setSp(35),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.red,
    fontWeight: FontWeight.bold,
  );

  static TextStyle h2title35Blue = TextStyle(
    fontSize: ScreenUtil().setSp(35),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.blue,
    fontWeight: FontWeight.bold,
  );

  static TextStyle h2title35yellow = TextStyle(
    fontSize: ScreenUtil().setSp(35),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.yellow,
    fontWeight: FontWeight.bold,
  );

  static TextStyle h2title35WhiteNormal = TextStyle(
    fontSize: ScreenUtil().setSp(35),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.white,
    fontWeight: FontWeight.normal,
  );

  static TextStyle h2title35RedNormal = TextStyle(
    fontSize: ScreenUtil().setSp(35),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.red,
    fontWeight: FontWeight.normal,
  );

  static TextStyle h2title35BlueNormal = TextStyle(
    fontSize: ScreenUtil().setSp(35),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.blue,
    fontWeight: FontWeight.normal,
  );

  static TextStyle h2title35yellowNormal = TextStyle(
    fontSize: ScreenUtil().setSp(35),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.yellow,
    fontWeight: FontWeight.normal,
  );

  //H3
  static TextStyle h3title20White = TextStyle(
    fontSize: ScreenUtil().setSp(20),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.white,
    fontWeight: FontWeight.bold,
  );

  static TextStyle h3title20Red = TextStyle(
    fontSize: ScreenUtil().setSp(20),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.red,
    fontWeight: FontWeight.bold,
  );

  static TextStyle h3title20Blue = TextStyle(
    fontSize: ScreenUtil().setSp(20),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.blue,
    fontWeight: FontWeight.bold,
  );

  static TextStyle h3title20Gray = TextStyle(
    fontSize: ScreenUtil().setSp(20),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.gray,
    fontWeight: FontWeight.bold,
  );

  static TextStyle h3title20GrayNormal = TextStyle(
    fontSize: ScreenUtil().setSp(20),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.gray,
    fontWeight: FontWeight.normal,
  );

  static TextStyle h3title20yellow = TextStyle(
    fontSize: ScreenUtil().setSp(20),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.yellow,
    fontWeight: FontWeight.bold,
  );

  static TextStyle h3title20WhiteNormal = TextStyle(
    fontSize: ScreenUtil().setSp(20),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.white,
    fontWeight: FontWeight.normal,
  );

  static TextStyle h3title20RedNormal = TextStyle(
    fontSize: ScreenUtil().setSp(20),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.red,
    fontWeight: FontWeight.normal,
  );

  static TextStyle h3title20BlueNormal = TextStyle(
    fontSize: ScreenUtil().setSp(20),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.blue,
    fontWeight: FontWeight.normal,
  );

  static TextStyle h3title20yellowNormal = TextStyle(
    fontSize: ScreenUtil().setSp(20),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.yellow,
    fontWeight: FontWeight.normal,
  );

//Paragraph 16
  static TextStyle paragraph16RedBold = TextStyle(
    fontSize: ScreenUtil().setSp(16),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.red,
    fontWeight: FontWeight.bold,
  );

  static TextStyle paragraph16BlueBold = TextStyle(
    fontSize: ScreenUtil().setSp(16),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.blue,
    fontWeight: FontWeight.bold,
  );

  static TextStyle paragraph16WhiteBold = TextStyle(
    fontSize: ScreenUtil().setSp(16),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.white,
    fontWeight: FontWeight.bold,
  );

  static TextStyle paragraph16GrayBold = TextStyle(
    fontSize: ScreenUtil().setSp(16),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.gray,
    fontWeight: FontWeight.bold,
  );

  static TextStyle paragraph16GrayNormal = TextStyle(
    fontSize: ScreenUtil().setSp(16),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.gray,
    fontWeight: FontWeight.normal,
  );

  static TextStyle paragraph16GrayLight = TextStyle(
    fontSize: ScreenUtil().setSp(16),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.grayLight,
    fontWeight: FontWeight.normal,
  );

  static TextStyle paragraph16White = TextStyle(
    fontSize: ScreenUtil().setSp(16),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.white,
    fontWeight: FontWeight.normal,
  );

  static TextStyle paragraph14Blue = TextStyle(
    fontSize: ScreenUtil().setSp(16),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.blue,
    fontWeight: FontWeight.bold,
  );

//Paragraph 14

  static TextStyle paragraph14Gray = TextStyle(
    fontSize: ScreenUtil().setSp(14),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.gray,
    fontWeight: FontWeight.normal,
  );

  static TextStyle paragraph14GrayBold = TextStyle(
    fontSize: ScreenUtil().setSp(14),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.gray,
    fontWeight: FontWeight.bold,
  );

  static TextStyle paragraph14White = TextStyle(
    fontSize: ScreenUtil().setSp(14),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.white,
    fontWeight: FontWeight.normal,
  );

  static TextStyle paragraph14WhiteBold = TextStyle(
    fontSize: ScreenUtil().setSp(14),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.white,
    fontWeight: FontWeight.bold,
  );

  static TextStyle paragraph12Red = TextStyle(
    fontSize: ScreenUtil().setSp(12),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.red,
    fontWeight: FontWeight.normal,
  );

  static TextStyle paragraph12RedBold = TextStyle(
    fontSize: ScreenUtil().setSp(12),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.red,
    fontWeight: FontWeight.bold,
  );

  static TextStyle paragraph12Blue = TextStyle(
    fontSize: ScreenUtil().setSp(12),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.blue,
    fontWeight: FontWeight.normal,
  );

  static TextStyle paragraph12BlueBold = TextStyle(
    fontSize: ScreenUtil().setSp(12),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.blue,
    fontWeight: FontWeight.bold,
  );

  static TextStyle paragraph12gray = TextStyle(
    fontSize: ScreenUtil().setSp(12),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.gray,
    fontWeight: FontWeight.normal,
  );

  static TextStyle paragraph12grayBold = TextStyle(
    fontSize: ScreenUtil().setSp(12),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.gray,
    fontWeight: FontWeight.bold,
  );

  static TextStyle paragraph12White = TextStyle(
    fontSize: ScreenUtil().setSp(12),
    letterSpacing: 1,
    fontFamily: 'Poppins',
    color: ThemeColors.white,
    fontWeight: FontWeight.normal,
  );
}

abstract class ThemeColors {
  static const Color white = Color.fromRGBO(
    250,
    251,
    250,
    1,
  );
  static const Color blue = Color.fromRGBO(
    0,
    159,
    251,
    1,
  );
  static const Color green = Color.fromARGB(
    255,
    42,
    175,
    2,
  );

  static const Color otherGreen = Color(
    0xff4BB543,
  );
  static const Color red = Color.fromRGBO(
    243,
    1,
    70,
    1,
  );
  static const Color yellow = Color.fromRGBO(
    254,
    147,
    28,
    1,
  );
  static const Color gray = Color.fromARGB(
    255,
    119,
    118,
    118,
  );
  static const Color grayLight = Color.fromARGB(
    255,
    187,
    186,
    186,
  );

  static const Color grayLight2 = Color.fromARGB(255, 235, 235, 229);
}
